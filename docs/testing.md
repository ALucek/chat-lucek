...
The Go suite starts one Postgres container, migrates it, and truncates tables between tests, so no local database is required. The e2e suite logs in through the real magic-link flow, reading the link back from a test-only endpoint served by a **fake** mailer. It keeps one test on **fake** Google Sign-In and uses a **fake** agent server, so it needs no real credentials or network.
